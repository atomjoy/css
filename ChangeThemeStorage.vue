...
function update() {
	document
		.querySelector('body')
		.setAttribute('color-scheme', th.value);
}

function toggle() {
	th.value = th.value == 'light' ? 'dark' : 'light';
	localStorage.setItem('theme-xxx-theme', th.value);
	update();
}
...
<template>
	<div
		@click="toggle"
		:class="{ 'change-theme': true }"
		:title="$t('Theme light')"
	>
		<div class="theme-icon theme-dark" v-if="th == 'dark'">
